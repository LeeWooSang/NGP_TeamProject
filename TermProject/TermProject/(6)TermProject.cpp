...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		...
		if (m.x >= Help.left && m.y >= Help.top && m.x <= Help.right && m.y <= Help.bottom)
		{   //차일드윈도우
			if (DlgOn == FALSE && !IsWindow(hDlg))
			{
				hDlg = CreateDialog(g_hInst, MAKEINTRESOURCE(IDD_DIALOG1), hwnd, DIALOG1Proc);
	...
	case WM_COMMAND:
		...
		{
		case ID_40001:
			isCheat = (isCheat == FALSE) ? TRUE : FALSE;
			break;
		case ID_40002: break;
		}
...
BOOL CALLBACK DIALOG1Proc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		hButton = GetDlgItem(hDlg, IDC_BUTTON1);                      // 버튼의 핸들 얻기
		SendMessage(hButton, BM_SETIMAGE, 0, (LPARAM)hBit);                // 버튼에 메시지 보내기
